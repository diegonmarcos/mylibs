...
{
//																				Allocate memory using ft_calloc from libft #debuging-line
	char	*ptr;
	int		fd;
	...
	if (ptr == NULL)
		write(1, "Alloc Error\n", 12);
//																				Open the file in append mode (create if it doesn't exist) #debuging-line
	fd = open(filename, O_CREAT | O_RDWR | O_APPEND, 0644);
	if (fd == -1)
		write(1, "Alloc Error\n", 12);
//																				Write the pointer address to the file #debuging-line
//																				Using a formatted string for readability and ease of parsing later #debuging-line
//																				using ";" as separator data in one line #debuging-line
//																				second and third lines are others arrays metadata(type, sizeof) #debuging-line
	if (ft_fprintf1(filename, "%p;", ptr) < 0)
		write(1, "Alloc Error\n", 12);
//																				Close the file #debuging-line
	close(fd);
	return (ptr);
}
...
{
//																				Open the file in read mode #debuging-line
	FILE	*file;
...
///																				Clean the file : Truncate to size 0 the file to reuse it #debuging-line
