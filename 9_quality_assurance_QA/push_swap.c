...
/*   Updated: 2025/01/16 20:01:47 by dinepomu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_list *ft_lstnew(int value, int index) 
{
    ...
    if (!new_node) 
	{
        ...
        halt_exit_(1);
		halt_exit(1, "265 - ft_lstnew memmory allocation failed"); //#debuging-line
    }
...
char	**check_args_1(int argc, char **argv)
{
	...
	{
		halt_exit_(1);
		halt_exit(1, "628 - less then 2 args"); //#debuging-line
	}
	else if(argc == 2)
	{
		argv_1 = ft_split(argv[1], ' ');
																				printf("array:");
																				for (int i = 0; argv_1[i]; i++){
																					printf("%s", argv_1[i]);
																				}printf("\n");
		check_args_2(argv_1);
	}
...
void radix_sort1(t_list **stack_a, t_list **stack_b, int size)
{
	...
	int		bit;
	t_list	*node;
																				print_linkedlist_debbuging(*stack_a, -1, "Stack A - Initial"); //#debuging-line
																				print_linkedlist_debbuging(*stack_b, -1, "Stack B - Initial"); //#debuging-line
	...
	while (i < 10)   															//max_bits=(9digits/32-bit/4 bytes for int, but whe made the index, so it will be max 3 digits/10 bits/2 bytes)
	{
		...
		while (j++ < size)
		{
			...
			{
				rotate("r", 'a', stack_a);
																				print_linkedlist_debbuging(*stack_a, j, "Stack A - Rotate"); //#debuging-line
			}
			else
			{
				push('b', stack_a, stack_b);
																				print_linkedlist_debbuging(*stack_a, j, "Stack A - Push B"); //#debuging-line
																				print_linkedlist_debbuging(*stack_b, j, "Stack B - Push B"); //#debuging-line
			}
		}
...
void	radix_sort(t_list **stack_a, t_list **stack_b, int size)
{
	...
	while (i < max_bits)
	{
		...
		i++;
																								...
																								printf("Stack AAA:\n");print_linked_list_field(*stack_a, "value"); //#debuging-line
...
int	main(int argc, char **argv)
{
    ...
    t_list *stack_b;
																				...
																				printf("###PUSH_SWAP###\n");	//#debuging-line
																				printf("###############\n\n");	//#debuging-line
																				printf("##0.Checking Args_1\n"); //#debuging-line
																				/* Check Inputs*/
	argv = check_args_1(argc, argv);
																				printf("##0.Normalizing Inputs\n"); //#debuging-line
	...
	stack_b = NULL;
																				print_linked_list_field(stack_a, "value");	//#debuging-line
																				printf("\n\n##1.Pre Sorting the Stack\n"); //#debuging-line
																				/* "Pre Sorting" Input(Selection Sort(get min)) and 
			adding the Indexing in the Linked List*/
	add_index_numbers(stack_a);
																				printf("__.Indexing is Done ✔️");	//#debuging-line
																				printf("\n###Array\n");	//#debuging-line
																				print_linked_list_field(stack_a, "value");	//#debuging-line
																				printf("###Index\n");	//#debuging-line
																				print_linked_list_field(stack_a, "index");	//#debuging-line
																				printf("\n\n##3.Sorting the Stack\n"); //#debuging-line
																				/* Sorting the Stack */
	stack_a=sort_stack(stack_a, stack_b);
																				is_sorted_linked_list(stack_a)?printf("__·Stack is Sorted ✔️\nStack A:\n"):printf("__Stack is NOT Sorted X\nStack A:\n") ; //#debuging-line
																				print_linked_list_field(stack_a, "value"); printf("Stack B:\n"); //#debuging-line
																				print_linked_list_field(stack_b, "value"); printf("\n"); //#debuging-line
